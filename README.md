# *Hierarchical-Web-Of-Hyper-Experts*
## Alexander J. Baron  
### Github: alexb02h

Current neural networks suffer from *Catastrophic Forgetting*, where learning a new task inadvertently overwrites the representations acquired from previous tasks. This "monolithic" approach to learning prevents AI from achieving true lifelong learning and cumulative knowledge.

This Research proposed the *Hierarchical Web of Hyper-Experts, a structured architecture designed to decouple task acquisition from knowledge retention. Instead of a single, rigid model, the architecture utilizes a *Parent Controller* that governs a decentralized cluster of specialized nerual experts.
